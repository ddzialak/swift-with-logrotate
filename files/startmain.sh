...
echo "Entering infinitive loop"
while true;
do
    echo "Rotate logs"
    /usr/sbin/logrotate /etc/logrotate.conf && pkill -f -SIGHUP rsyslogd || true

    echo -n "Removing empty files: "
    ionice -c idle find /srv/sdb1 -mindepth 2 -mtime +7 -type f -size 0 -delete -print 2> /dev/null | wc

    echo -n "Removing directories: "
    ionice -c idle find /srv/sdb1 -mindepth 2 -mtime +7 -type d -delete -print 2> /dev/null | wc
